...
import core.sinks.KafkaDStreamSink
...
object Prefilter {
  ...
  def classify(
    ...
    val bProducerConfig: Broadcast[Map[String, Object]] = sc.broadcast(producerConfig)
        ...
        val metadata = partition
          ...
          logLine match{
           /* case loadRegexPrefilter(loadHead,loadTail) => new ProducerRecord[String,String](topicLoadLog,"load",logLine)
            ...
            case loadRegexPrefilter(loadHead,loadTail) => new ProducerRecord[String,String](topicLoadLog,UUID.randomUUID().toString,logLine)
            case conRegexPrefilter(conHead,conTail)    => new ProducerRecord[String,String](topicConLog,UUID.randomUUID().toString,logLine)
            case _                                     => new ProducerRecord[String,String](topicError,UUID.randomUUID().toString,logLine)
          }
        }.map{ record =>
