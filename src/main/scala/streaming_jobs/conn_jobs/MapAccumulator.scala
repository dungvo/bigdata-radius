...
/**
  ...
  * refer http://stackoverflow.com/questions/18660769/best-practices-for-mixing-in-scala-concurrent-map
  * @param accumulator
  */
  ...
  extends AccumulatorV2[(String, Int), mutable.Map[String, Int]]
{
  ...
  def this(){
    ...
    val  timer  =  new  Timer()
    timer.scheduleAtFixedRate(runner,0, 10000L) // not work
  ...
  class RunCheckReset extends  TimerTask{
   ...
   override def run(): Unit = {
     ...
     //val midnight = new DateTime(2017,4 ,25 ,15 ,50 ,0 ,0); // testing purpose
