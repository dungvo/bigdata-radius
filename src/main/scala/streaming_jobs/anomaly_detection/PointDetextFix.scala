...
/**
  ...
  */
class PointDetextFix (config: PointDetectConfig,source: KafkaDStreamSource) extends SparkStreamingApplication{
  ...
  def detect(backTime: Int): Unit = {
    withSparkStreamingContext{
      (sc,ssc) =>
        ...
        DetectAnomalyFixJob.detect(ssc,
          sc,
          input,
          //config.windowDuration,
          //config.slideDuration,
...
object PointDetecFixExe{
  def main(args: Array[String]): Unit = {
    ...
    val pointDetect = new PointDetextFix(config,KafkaDStreamSource(config.sourceKafka))
    pointDetect.detect(24+24 + 24 + 6)
  }
}
