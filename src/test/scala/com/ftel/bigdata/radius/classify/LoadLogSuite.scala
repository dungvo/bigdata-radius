...
class LoadLogSuite extends FunSuite {
  ...
  test("TEST PARSE LOAD LOG Full Format") {
    ...
    val log = LoadLog(line, "2018-01-31")
    assert(log.isInstanceOf[LoadLog])
    val loadLog = log.asInstanceOf[LoadLog]
    assert(loadLog.statusType == "ACTALIVE")
    assert(DateTimeUtil.create(loadLog.timestamp).toString(LoadLog.DATE_TIME_PATTERN) == "Dec 01 2017 06:59:59")
  ...
  test("TEST PARSE LOAD LOG Patial Format") {
    ...
    assert(DateTimeUtil.create(loadLog.timestamp).toString(LoadLog.DATE_TIME_PATTERN) == "Dec 01 2017 19:57:53")
    ...
    assert(loadLog.inputG == -1)
    assert(loadLog.outputG == -1)
    
    assert(loadLog.inputIPv6 == -1)
    assert(loadLog.inputIPv6G == -1)
    assert(loadLog.outputIPv6 == -1)
    assert(loadLog.outputIPv6G == -1)
  }
}
