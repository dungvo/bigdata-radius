...
class FunctionssSuite extends FunSuite {
  test("TEST calculateLoad") {
    ...
    val contract1 = rows
      ...
      .map(x => x.sessionId -> (x.download, x.upload))
  ...
  test("TEST new calculateLoad") {
    ...
    //loadStats.filter(x => x.name == "Lddsl-161001-360").foreach(println)
    
    val rows = Functions.calculateLoad(sparkSession, loadStats)
    ...
    val contract1 = rows
      ...
      .collect()
      .toMap
    assert(contract1.get("1").get._1 == 120)
    assert(contract1.get("1").get._2 == 0)
    ...
    val contract2 = rows
      ...
      .collect()
      .toMap
    assert(contract2.get("1").get._1 == 120)
    assert(contract2.get("1").get._2 == 30)
